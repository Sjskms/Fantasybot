...
from keyboards.session_kb import (
    ...
    #get_session_detailed_stats_keyboard,
    get_session_stats_keyboard,
)

def get_detailed_stats_keyboard(session_name: str, current_page: int, total_pages: int) -> InlineKeyboardMarkup:
    keyboard = []
...
async def session_detailed_stats_handler(callback: CallbackQuery):
    ...
    if not channels_dict:
        text = (
            f"📊 <b>Подробная статистика сессии:</b> <code>{escape(session_name)}</code>\n\n"
    ...
    for ch_id, ch_data in page_channels:
        ...
        flt = stats.get("filtered", 0)
        err = stats.get("errors", 0)

        text += (
            f"📢 <b>{escape(title)}</b> [<code>{ch_id}</code>]\n"
            f"  ├ 📤 Переслано: <b>{fwd}</b>\n"
            f"  ├ 🚫 Отфильтровано: <b>{flt}</b>\n"
            f"  └ ❌ Ошибок: <b>{err}</b>\n\n"
        )

    keyboard = get_detailed_stats_keyboard(session_name, current_page, total_pages)
    try:
        await callback.message.edit_text(text, reply_markup=keyboard, parse_mode="HTML")
    except TelegramBadRequest:
        pass
